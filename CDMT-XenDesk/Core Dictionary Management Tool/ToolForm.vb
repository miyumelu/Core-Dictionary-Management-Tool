...
Public Class ToolForm
    ...
    Private Sub Tree()
        Dim mainStrip As New List(Of MenuItemData)

        ' Main Strip
        Dim appItem = New MenuItemData("CDMT", "id_cdmt")

        Dim fileItem = New MenuItemData("File", "id_file")
        Dim lteItem = New MenuItemData("Light-Term Encryption", "id_lte")

        ' App Subitems

        ' Tree Subitems
        Dim treeItem = New MenuItemData("Tree", "id_tree") ' Main

        Dim sysTreeSub = New MenuItemData("System Tree", "id_systree")

        Dim sysApps = New MenuItemData("System Application", "cmd_sapp")
        Dim sysExt = New MenuItemData("System Extention", "cmd_sext")
        Dim root = New MenuItemData("Root", "cmd_root")

        Dim manualTreeSub = New MenuItemData("Manual Tree", "cmd_mtree")
        Dim refreshTreeSub = New MenuItemData("Refresh Tree", "cmd_rtree")

        sysTreeSub.SubItems.Add(sysApps)
        sysTreeSub.SubItems.Add(sysExt)
        sysTreeSub.SubItems.Add(root)

        treeItem.SubItems.Add(sysTreeSub)
        treeItem.SubItems.Add(manualTreeSub)
        treeItem.SubItems.Add(refreshTreeSub)

        'Sender
        mainStrip.Add(appItem)
        mainStrip.Add(treeItem)

        _client.SendMenu(mainStrip)
    End Sub
End Class